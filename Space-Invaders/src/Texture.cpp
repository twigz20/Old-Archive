	...
	game(Game::getInstance())
{
	...
	else
	{
		...
		texture_ = SDL_CreateTextureFromSurface( Game::getInstance().renderer, loadedSurface );
		if( texture_ == nullptr )
		{
...
void Texture::render(int x, int y, SDL_Rect* clip, double angle, SDL_Point* center, SDL_RendererFlip flip) const
{
	...
	//Render to screen
	SDL_RenderCopyEx( game.renderer, texture_, clip, &renderQuad, angle, center, flip );
}

void Texture::render( SDL_Rect* src, SDL_Rect* dst )
{
    SDL_RenderCopy( game.renderer, texture_, src ,dst);
}
