...
{
	std::map<std::string, Mix_Music*>::iterator i = music.find(filename);
	if(i == music.end())
	{
		Mix_Music *tempMus = Mix_LoadMUS( filename.c_str() );
		if (tempMus == nullptr)
			throw std::runtime_error("Error while sound: " + filename + " " + Mix_GetError());

		music.insert(std::pair<std::string, Mix_Music*>(filename, tempMus));
